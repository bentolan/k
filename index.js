		...
		try {
			...
			switch(command) {
				...
				case 'infogempa':
					...
					reply(mess.wait)
					anu = await fetchJson(`https://mhankbarbar.herokuapp.com/api/infogempa?apiKey=${apiKey}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					const gmpa = `*Kedalaman* : ${anu.kedalaman\n*Koordinat* : ${anu.koordinat}\n*Lokasi* : ${anu.lokasi}\n*Magnitude* : ${anu.magnitude}\n*Potensi* : ${anu.potensi}\n*Waktu* : ${anu.waktu}`
					gpict = await getBuffer(anu.map)
					client.sendMessage(from, gpict, image, {quoted: mek, caption: `${gmpa}`})
					break
				case 'infoig':
					if (args.length < 1) return reply('Masukan username nya cuyy')
					reply(mess.wait)
				...
				case 'jadwalshalat':
					if (args.length < 1) return reply('Teksnya mana cuyy')
					teks = body.slice(14)
					anu = await fetchJson(`https://mhankbarbar.herokuapp.com/api/jadwalshalat?daerah=${teks}&apiKey=${apiKey}`, {method: 'get'})
				...
				case 'xnxx':
					if (!isOwner) return reply('*Premium Only.*\nChat jika ingin fitur Premium : *wa.me/6281283883549*')
					if (args.length < 1) return reply('Linknya mana cuyy')
					if(!isUrl(args[0]) && !args[0].includes('xnx')) return reply(mess.error.Iv)
					reply(mess.wait)
					anu = await fetchJson(`https://mhankbarbars.herokuapp.com/api/xnxx?url=${args[0]}&apiKey=${apiKey}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					teks = `*Judul* : ${anu.result.judul}\n*Size* : ${anu.result.size}`
					thumb = await getBuffer(anu.result.thumb)
					client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result.vid)
					client.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', filename: `${anu.result.judul}.mp4`, quoted: mek})
					break
				case 'ytsearch':
